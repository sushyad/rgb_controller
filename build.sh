...
export PLATFORMIO_SRC_BUILD_FLAGS="'-DSTA_PASS=\"$WIFI_PASS\"' '-DSTA_SSID=\"$WIFI_SSID\"' '-DMQTT_HOST=\"$MQTT_HOST\"' '-DOTA_HOST=\"$OTA_HOST\"' '-DOTA_PASS_MD5_HASH=\"$OTA_PASS_MD5_HASH\"' '-DPROJECT_NAME=\"$PROJECT_NAME\"' '-DVERSION_BIN=\"$VERSION\"'"

if [ $# -eq 0 ]
then
    FILENAME=${PROJECT_NAME}-${VERSION}.bin
