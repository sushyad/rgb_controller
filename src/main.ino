...
void mqtt_connected()
{
  ...
  mqttConnectedDeviceCallback(mqttClient);
...
void mqttDataCb(char* topic, byte* data, unsigned int data_len)
{
  ...
  if (type != NULL) {
    ...
    if (!strcmp(type,"STATUS")) {
      ...
      if ((payload == 0) || (payload == 8)) {
        snprintf_P(svalue, sizeof(svalue), PSTR("%s, %s, %s, %s, %d, %d"),
          Version, sysCfg.mqtt_topic, sysCfg.mqtt_topic2, sysCfg.mqtt_subtopic, sysCfg.timezone, sysCfg.ledstate);
    ...
    } else if (!deviceCallback(topic, data, data_len)) {
      type = NULL;
    }
    if (type == NULL) {
      blinks = 1;
